 ...
 * @constant {Element} videoControls - The parent-element that holds the above button HTMLElement.
 * 
 */
...
const srPlayText = '<span class="visually-hidden">Play background decorative video</span>';
const srPauseText = '<span class="visually-hidden">Pause background decorative video</span>';
const playIcon = '<svg id="play" class="img-fluid" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 56 56"><path d="M45.27,27.16L11.41,7.61c-0.65-0.37-1.45,0.09-1.45,0.84v39.1c0,0.75,0.81,1.21,1.45,0.84 l33.86-19.55C45.92,28.47,45.92,27.53,45.27,27.16z"/></svg>';
const pauseIcon = '<svg id="pause" class="img-fluid" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 56 56"><path d="M45.07,49.28H34.93c-0.54,0-0.97-0.43-0.97-0.97V7.69c0-0.54,0.43-0.97,0.97-0.97h10.15 c0.54,0,0.97,0.43,0.97,0.97v40.62C46.04,48.84,45.61,49.28,45.07,49.28z"/><path d="M21.07,49.28H10.93c-0.54,0-0.97-0.43-0.97-0.97V7.69c0-0.54,0.43-0.97,0.97-0.97h10.15 c0.54,0,0.97,0.43,0.97,0.97v40.62C22.04,48.84,21.61,49.28,21.07,49.28z"/></svg>';

function setButtonState(button, video, isInitialBtnState) {
  const videoIsPaused = video.paused == true;
  let buttonContents = videoIsPaused ? srPauseText + pauseIcon
    : srPlayText + playIcon;

  button.innerHTML = buttonContents;

  if (isInitialBtnState) return;
...
function makeVideoControlsVisible(videoControls, button) {
...
function createVideoControls() {
  ...
  makeVideoControlsVisible(videoControls, button);
}
